...
func drawNavButton(dst *ebiten.Image, label string, x, y, w, h float32, focused bool, iconFn func(*ebiten.Image, float32, float32, float32, color.Color), accentColor color.Color) {
	if focused {
		...
		DrawTextCentered(dst, label, float64(x+w/2+8), float64(y+h/2), navFontBody, ColorBackground)
		if iconFn != nil {
		...
		DrawTextCentered(dst, label, float64(x+w/2+8), float64(y+h/2), navFontBody, ColorText)
		if iconFn != nil {
